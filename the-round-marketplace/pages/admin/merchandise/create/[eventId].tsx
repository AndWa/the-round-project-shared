...
type MerchandiseListing = BaseListingFormValues & FormValues;

export interface MerchandiseCreateProps {
  onCreate?: () => void;
}

const CreateMerchandise = ({ onCreate }: MerchandiseCreateProps) => {
  ...
  useEffect(() => {
    ...
    const getTransactionStatus = async (hash: string, accountId: string) => {
      ...
      await createMerchandise(tokenSeriesId);
    };
  ...
  const form = useForm<MerchandiseListing>({
    initialValues: {
      isActive: true,
      title: "",
      media: "",
  ...
  const createMerchandise = async (tokenSeriesId: string | null) => {
    setIsLoading(true);
    ...
    showNotification({
      ...
      loading: true,
      disallowClose: true,
      title: "Merchandise Creation",
      message: "Please wait we are creating a merchandise...",
      icon: <Check />,
    });
    ...
    try {
      ...
      if (!tokenSeriesId) {
        ...
        if (mediaFile) {
          const imagePath = await saveToBucket(
            `merchandise/${create.slug}/media-${create.slug}`,
            mediaFile
          );
      ...
      updateNotification({
        ...
        loading: false,
        title: "Merchandise Creation",
        message: "There was an error creating the merchandise",
        icon: <X />,
        color: "red",
      });
    } finally {
  ...
  return (
    <Box mx="auto">
      <Title mb={8}>Create new Merchandise</Title>
        ...
        <fieldset style={{ border: 0 }} disabled={isLoading}>
          <Stack spacing={16}>
            ...
            <NumberInput
              ...
              label="Stock"
              description="Enter the number of items available (if 0 then unlimited)"
            ...
            <DatePicker
              ...
              label="Merchandise sale start date"
              required
              onChange={(date) => {
            ...
            <TimeInput
              ...
              label="Merchandise sale start time"
              required
              onChange={(date) => {
            ...
            <DatePicker
              ...
              label="Merchandise sale end date"
              required
              onChange={(date) => {
            ...
            <TimeInput
              ...
              label="Merchandise sale end time"
              required
              onChange={(date) => {
                const newDate = combineTimeAndDate(
                  date,
            ...
            <Switch
              label="Is this merchandise active?"
              size="lg"
              onLabel="Yes"
              offLabel="No"
            ...
            <TextInput
              ...
              {...form.getInputProps("merchandise.model")}
            />

            <Switch
              label="Is this merchandise equippable?"
              size="lg"
              onLabel="Yes"
              offLabel="No"
              {...form.getInputProps("merchandise.equippable", {
                type: "checkbox",
              })}
            />
